...
import { cleanClinicsError } from "../../../../store/clinics/actions";
...
const AdminPatientDetail = (props) => {
  ...
  const { my_patient, patient_treatments, error, loading } = useSelector((state) => ({
  ...
  const { errorClinics } = useSelector((state) => ({
  ...
  const handleDisablePatient = () => {
    if (my_patient?.guid) dispatch(disableMyPatient(my_patient.guid));
    setDisablePatientModal(false);
  };
  const handleEnablePatient = () => {
    if (my_patient?.guid) dispatch(enableMyPatient(my_patient.guid));
  ...
  const validation = useFormik({
    ...
    onSubmit: (values) => {
      dispatch(updateMyPatient({ guid: patientItem.guid, name: values.name, email: values.email, disabled: patientItem.disabled }));
  ...
  return (
    <React.Fragment>
      ...
      <QuestionModal
        ...
        positive={false}
        questionText={`${props.t("Are you sure you want to disable the patient")}: ${my_patient?.email}`}
      ...
      <QuestionModal
        ...
        positive={true}
        questionText={`${props.t("Are you sure you want to enable the patient")}: ${my_patient?.email}`}
      ...
      <div className="page-content">
        <Container fluid>
          ...
          {errorClinics && showToast(props.t("Error"), getErrorMessage(props.t, errorClinics), () => dispatch(cleanClinicsError()))}
                ...
                <CardBody>
                  <CardTitle className="mb-4 d-flex justify-content-between align-items-center">
                    ...
                    <button
                      ...
                      onClick={() => { setPatientItem(my_patient); setEditModal(true); }}
                    >
                        ...
                        <tr>
                          ...
                          <td>
                            <div className="input-group" style={{ maxWidth: 280 }}>
                              <Input
                                value={my_patient?.patient_mobile_accesses?.[0]?.credential ?? ""}
                                type={pwType}
                                disabled
                                readOnly
                              />
