...
import { 
  ...
  createClinicClinicianByAdmin, 
  cleanClinicianError,
  activateClinicClinicianByAdmin,
...
const AdminClinicianDetail = (props) => {
  ...
  const { my_clinics } = useSelector((state) => ({ my_clinics: state.clinics.my_clinics }));
  ...
  const validation = useFormik({
    ...
    onSubmit: (values) => {
      ...
      if (isValidClinic(clinic)) {
        ...
        dispatch(createClinicClinicianByAdmin(data));  
      }
  ...
  useEffect(() => {
    ...
    dispatch(getClinicianByAdmin(guid));
    dispatch(getClinicianClinicsByAdmin({guid, disabled: false}));
  }, []);
  ...
  const handleDisableRelationship = () => {
    if (relationship) {
      dispatch(activateClinicClinicianByAdmin({ 
  ...
  const getClinicsLabels = () => {
    const availableClinics = my_clinics.filter(clinic => {
      if (clinic.clinic_clinicians[0]?.admin === true) {
...
AdminClinicianDetail.propTypes = {
  router: PropTypes.any,
  t: PropTypes.any,
  guid: PropTypes.string,
};

export default withRouter(withTranslation()(AdminClinicianDetail));
