...
import { FormFeedback, Input, Label, Modal, ModalBody } from "reactstrap";
...
import { getClinics, getMyClinics } from "../../../../store/actions";
...
const NewOriginalStimulation = (props) => {
  ...
  const validation = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      //if (!check) return;
      const myclinic = !selectedClinic.guid ? my_clinics[0].guid : selectedClinic.guid;
      const data = {
        original: false,
        type: type,
        title: values.title,
  ...
  const checkStimulationName = async (name) => {
    /*try {
      ...
      const [data] = await Promise.all([
        await get(`${baseurl}/clinician/check-stimulations?name=${name}`, true, {}),
      ])
      if(data){
